...
def main_function(input_dim=32, n_channel=3, num_kernels=[32, 32, 32, 32, 64, 64, 64, 128, 128, 128],
                  ...
                  batch_size=50, epochs=10, lr=0.0002, lr_end = 0.0001, kl_factor=0.00001, Testing_SV=True,
                  Random_noise=False, Adversarial_noise=False, HCV=0.1,  Black_box_attack=False,  corrupted_images=False,kind="brightness",    
                  PGDBlack_box_attack=False, adversary_target_cls=3, Targeted=False, PGD_Adversarial_noise=False, stepSize=1, maxAdvStep=40,
                  Training=False, Testing=False, continue_training=False, saved_model_epochs=350):
    PATH = './UT_VDP_c10/saved_models/UTVDP_cnn_epoch_{}/'.format(epochs) 
...
##                       }) 
    ...
    # -------------------------Testing-----------------------------
       ...
       #     print('SNR', np.mean(snr_signal)) 
        ...
        for i in range(int(valid_size /batch_size)):
            for j in range(batch_size):               
                ...
                if (predicted_out[k] == true_out[k]):
                    ...
                    correct_classification1 +=  sigma_[i,j, int(predicted_out[k]), int(predicted_out[k])] 
                    k1=k1+1
                if (predicted_out[k] != true_out[k]):
                    misclassification_pred[k2] = sigma_[i,j, int(predicted_out[k]), int(predicted_out[k])] 
                    misclassification_pred1 +=  sigma_[i,j, int(predicted_out[k]), int(predicted_out[k])] 
                    misclassification_true[k2] = sigma_[i,j, int(true_out[k]), int(true_out[k])]   
                    misclassification_true1 += sigma_[i,j, int(true_out[k]), int(true_out[k])]
                    k2=k2+1                 
                k=k+1         
        ...
        misclassification_true2 = misclassification_true1/k2    
...
#    main_function(Testing=True,Random_noise=True, Adversarial_noise=False, HCV=0.01)
#    main_function(Testing=True,Random_noise=True, Adversarial_noise=False, HCV=0.05)
#    main_function(Testing=True,Random_noise=True, Adversarial_noise=False, HCV=0.1)
#    
#    
#    main_function(Testing=False,Random_noise=False, Adversarial_noise=True,Targeted=True, HCV=0.01)
#    main_function(Testing=False,Random_noise=False, Adversarial_noise=True,Targeted=True, HCV=0.05)
#    main_function(Testing=False,Random_noise=False, Adversarial_noise=True,Targeted=True, HCV=0.1)
#    
#    main_function(Testing=False,Random_noise=False, Adversarial_noise=True,Targeted=False, HCV=0.01)
#    main_function(Testing=False,Random_noise=False, Adversarial_noise=True,Targeted=False, HCV=0.05)
#    main_function(Testing=False,Random_noise=False, Adversarial_noise=True,Targeted=False, HCV=0.1)
#    main_function(Random_noise=False, Adversarial_noise=True,Targeted=True, HCV=0.02)
#    main_function(Random_noise=False, Adversarial_noise=True,Targeted=True, HCV=0.03)
#    main_function(Random_noise=False, Adversarial_noise=True,Targeted=True, HCV=0.04)
#    main_function(Random_noise=False, Adversarial_noise=True,Targeted=True, HCV=0.045)
#    main_function(Random_noise=False, Adversarial_noise=True,Targeted=True, HCV=0.06)
#    
#    #main_function(Random_noise=False, Adversarial_noise=True,Targeted=False, HCV=0.01)
