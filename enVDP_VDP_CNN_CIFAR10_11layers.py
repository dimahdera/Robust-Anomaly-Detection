...
class ensemble_ELU(keras.layers.Layer):
    def __init__(self):
        super(ensemble_ELU, self).__init__()
    def call(self, mu_in, Sigma_in):
        batch_size = mu_in.shape[0]
        n_g =  mu_in.shape[1]*mu_in.shape[1]
        n_s =  mu_in.shape[1]*mu_in.shape[1]
        Sigma_in1 = tf.transpose(Sigma_in, [0,3, 1,2])
        ...
        x_hat = tf.transpose(tf.reshape(mu_in, [batch_size, n_g, -1]), [0,2,1])#shape=[batch_size, num_filter[0], image_size*image_size]
        dist = tfd.MultivariateNormalDiag(loc=x_hat ,scale_diag=L_gg)
        ensemble= dist.sample(sample_shape=(n_s), dtype=tf.float32, seed=1) #shape=[n_s,batch,num_filter[0], image_size*image_size]
        ensemble_trans= tf.nn.elu(ensemble)

        mu_g1 = tf.reduce_mean(ensemble_trans, 0) #shape [ batch, num_filters,image*image]
...
class ensemble_ELU_full(keras.layers.Layer):
    def __init__(self):
        super(ensemble_VDPELU_full, self).__init__()
    def call(self, mu_in, Sigma_in):
        batch_size = mu_in.shape[0]
        n_s =  2*mu_in.shape[1]*mu_in.shape[1]
        Sigma_in1 = tf.transpose(Sigma_in, [0,3, 1,2])
        x_hat = tf.transpose(tf.reshape(mu_in, [batch_size, n_s, -1]), [0,2,1])#shape=[batch_size, num_filter[0], image_size*image_size]
        dist = tfd.MultivariateNormalFullCovariance(loc=x_hat ,scale_diag=Sigma_in1)
        ensemble= dist.sample(sample_shape=(n_s), dtype=tf.float32, seed=1) #shape=[n_s,batch,num_filter[0], image_size*image_size]
        ensemble_trans= tf.nn.elu(ensemble)
...
def main_function(input_dim=32, n_channel=3, num_kernels=[32, 32, 32, 32, 64, 64, 64, 128, 128, 128],
                  ...
                  batch_size=50, epochs=2, lr=0.00001, lr_end = 0.000001, kl_factor=0.00001,Testing_SV=False,
                  Random_noise=False, Adversarial_noise=False, HCV=0.1, corrupted_images=False,kind="canny", Black_box_attack=False, 
                  PGDBlack_box_attack=False, adversary_target_cls=3, Targeted=False, PGD_Adversarial_noise=False, stepSize=1, maxAdvStep=40,
                  Training=False, Testing=False, continue_training=False, saved_model_epochs=250):
    PATH = './EN_saved_models_CIFAR10/epoch_{}/'.format(epochs) 
  #  PATH = '///saved_models_CIFAR10/epoch_{}/ensemble_VI/'.format(epochs) 
    ...
    if Training: 
        if continue_training:
            saved_model_path =  '/data/giuse/FUSION_2022/saved_models_CIFAR10/epoch_{}/ensemble_VI/'.format(saved_model_epochs) 
