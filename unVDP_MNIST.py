...
def main_function(input_dim=28, num_kernels=[32], kernels_size=[5], maxpooling_size=[2], maxpooling_stride=[2], maxpooling_pad='SAME', class_num=10 , batch_size=50,
        ...
        Random_noise=False, gaussain_noise_std=0.5, Adversarial_noise=False, epsilon = 0.05,corrupted_images=False,kind="Canny", adversary_target_cls=3, Targeted=False, Testing_F=True,
        Training = False, continue_training = False,Testing=False,  saved_model_epochs=50):   
       
    PATH = './UT_saved_models/UT_cnn_epoch_{}/'.format( epochs)   
...
#    ((initial_learning_rate - end_learning_rate) * (1 - step / decay_steps) ^ (power) ) + end_learning_rate
    ...
    def create_adversarial_pattern(input_image, input_label):
          ...
          return signed_grad         
    if Training:
        wandb.init(entity = "oruneaminul", project="UT_VDP_CNN_mnist_epochs_{}_lr_{}_lambda03".format(epochs, lr))
        if continue_training:
            saved_model_path = './UT_saved_models/UT_cnn_epoch_{}/'.format(saved_model_epochs)
            cnn_model.load_weights(saved_model_path + 'ut_cnn_model')
        ...
        for epoch in range(epochs):
            ...
            cnn_model.save_weights(PATH + 'ut_cnn_model')   
    ...
    if (Testing_F):
        ...
        x_test = tf.expand_dims(x_test, -1)
        val_dataset = tf.data.Dataset.from_tensor_slices((x_test, one_hot_y_test)).batch(batch_size)
        cnn_model.load_weights(PATH + 'ut_cnn_model')
        test_no_steps = 0        
